...
class Utils {
  ...
  static Color colorRandom({double opacity = 1}) {
    ...
    return Color.fromRGBO(
        rand.nextInt(255), rand.nextInt(255), rand.nextInt(255), opacity);
  }

  static void drawIcon(Canvas canvas, IconData icon, Offset drawPoint,
      {Color color = Colors.green, double size = 30}) {
    final paragraph = ui.ParagraphBuilder(
        ui.ParagraphStyle(fontSize: size, textAlign: TextAlign.center));

    paragraph.pushStyle(
        ui.TextStyle(color: Colors.white, fontFamily: icon.fontFamily));

    paragraph.addText(String.fromCharCode(icon.codePoint));

    canvas.drawParagraph(
        paragraph.build()
          ..layout(ui.ParagraphConstraints(width: Size.infinite.width)),
        drawPoint);
  }
